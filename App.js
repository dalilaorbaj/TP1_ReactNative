...
export default function App() {
  ...
  const handleMensaje = (text) => {
    setMensaje(text)
  }
  return (
    <View style={styles.container}>
      <View style={styles.lilContainer}>
        <TextInput placeholder='Ingrese texto' onChangeText={(text) => handleMensaje(text)} style={{ alignSelf: 'center'}}></TextInput>
        <Text style={{ alignSelf: 'center', marginTop:5}}>Caracteres:{mensaje.length}</Text>
